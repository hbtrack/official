...
- Enforcement server‑side (CI + branch protection) ainda não está comprovado neste workspace (não há `.github/workflows/`).
...
Conclusão: o repo está em maturidade “piloto com 1 módulo forte”, o que é aceitável **desde que** o pipeline seja determinístico e CI seja obrigatório (hoje não é).

### 4.3 Evidência objetiva encontrada (datas fixas)
- `_reports/contract_gates/latest.json` (**execução no WSL — 2026-03-14T04:15:04Z**):
  ...
  - observação: `oasdiff` WSL‑native não foi comprovado (o `oasdiff` encontrado tende a ser wrapper Windows em `$HOME/bin`); o gate de breaking changes usa fallback determinístico (detecta remoção de operações method+path) quando `oasdiff` não é utilizável no WSL.
...
- `CHECKLIST_EXECUTIVO.md`:
  - status global: **FAIL**
  - data referência: **2026-03-13**
  - lista motivos: toolchain, enforcement não comprovado, runtime mismatch sem prova, etc.
...
#### P0‑3) Ausência de CI (nenhum `.github/workflows/`)
**Problema:** sem enforcement server‑side, governança depende de disciplina local.  
**Impacto:** “CDD” vira guideline. Qualquer bypass local quebra o sistema.

#### P0‑4) Waivers machine‑readable exigidos no canon, mas pasta inexistente
**Problema:** `docs/_canon/CI_CONTRACT_GATES.md` define `contracts/_waivers/`, mas a pasta não existe.  
**Impacto:** exceções governadas não são possíveis; a cultura tende a criar bypass textual.
...
**Gap remanescente (para “hermeticidade real” em CI):**
- garantir dependências pinadas e instaláveis por projeto (`npm ci` + devDependencies explícitas para CLIs Node);
- prover `oasdiff` WSL‑native no ambiente de CI (ou manter fallback apenas como contingência, não como padrão).
...
**Ação:** alinhar policy compiler + manifests para referenciar paths SSOT existentes (ou criar alias canônico com ADR).  
**Critério de aceite:** `generated/manifests/*.traceability.yaml` e `generated/resolved_policy/*.resolved.yaml` só apontam para arquivos existentes + hashes corretos.

### 7.3 P0: Implementar waivers machine‑readable (ou remover a exigência do canon)
**Ação:** criar `contracts/_waivers/<gate_id>/*.json` + validador + gate de validade.  
**Critério de aceite:** breaking changes e exceções só passam com waiver válido (expiração + fingerprint).

### 7.4 P0/P1: Adicionar CI e tornar gates obrigatórios (server‑side)
**Ação:** criar workflow(s) que rodem:
- `python3 scripts/validate_contracts.py`
- `python3 scripts/contracts/validate/api/compile_api_policy.py --all --check`
- `redocly lint ...`
- `spectral lint ...`
- `asyncapi validate ...` (quando aplicável)
- `oasdiff breaking ...` (com baseline governado)

**Critério de aceite:** PR não mergeia sem PASS (required checks).

### 7.5 P1: Toolchain hermética (pinning + execução via projeto)
**Node**
- mover CLIs para `devDependencies` e usar `npm run gate:*` (ou `npx` com versão fixa).

**Python**
- criar lock canônico com pins (PyYAML/jsonschema/schemathesis etc.).

**Critério de aceite:** CI roda do zero sem instalações globais.
...
- `contracts/_waivers/` (waivers exigidos no canon, pasta ausente)
...
- adicionar CI obrigatório + branch protection,
- pin/lock toolchain,
...
Implementar **enforcement server‑side**:
- criar `.github/workflows/` para rodar `python3 scripts/validate_contracts.py` em PR/push e publicar `_reports/contract_gates/` como artefato;
- configurar **required checks + branch protection** no GitHub;
- adicionar `CODEOWNERS` para SSOT e runner de gates (`.contract_driven/**`, `contracts/**`, `scripts/contracts/validate/**`, `scripts/validate_contracts.py`).
