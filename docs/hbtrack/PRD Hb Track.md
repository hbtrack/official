...
**Vínculo Direto**:  tabela de classificações (competition_standings) deve possuir uma relação direta com a entidade teams (Nossa Equipa).

**Visualização**: O sistema deve ser capaz de filtrar e destacar a posição da equipa do utilizador na tabela de classificação sem necessidade de mapeamentos manuais como "time adversário".

**Consistência**: A query de classificação deve garantir que, se a nossa equipe estiver inscrita na competição, ela apareça na tabela mesmo que o linked_team_id na tabela de adversários seja nulo.
...
- **Obrigatoriedade:** O campo birth_date é obrigatório para todas as pessoas classificadas como Atletas.

- **Paridade:** O sistema deve garantir, via triggers de banco de dados e validação em service, que persons.birth_date seja idêntico a athletes.birth_date.

- **Integridade:** É proibida a existência de um registro em athletes cujo person_id aponte para uma Person com birth_date nulo.
