...
## Critérios de Aceite
1) docs/hbtrack/ars/_INDEX.md existe. 2) Todos os AR_XXX IDs presentes em AR_*.md no disco têm entrada correspondente em _INDEX.md. 3) Contagem de ARs em disco == linhas de dados no _INDEX.md (excluindo header). 4) _INDEX.md contém header '⚠️ Auto-gerado'. 5) Nenhum AR_*.md ficou de fora do index.
...
`docs/hbtrack/evidence/AR_033_ar_index_validation_checkpoint.log`

## Notas do Arquiteto
TESTES DETERMINÍSTICOS (formato Manual Deterministico):

GATE-INDEX-001: Completude
  cmd: python -c "import pathlib,re; ar_dir=pathlib.Path('docs/hbtrack/ars'); files=[f for f in ar_dir.glob('AR_*.md') if '_INDEX' not in f.name]; ar_ids=set(); [ar_ids.add(m.group(1)) for f in files for m in [re.match(r'AR_([0-9]{3})',f.name)] if m]; idx=(ar_dir/'_INDEX.md').read_text(encoding='utf-8'); missing=[i for i in sorted(ar_ids) if f'AR_{i}' not in idx]; assert not missing,f'FAIL: {missing}'; print(f'PASS: {len(ar_ids)} ARs no index')"
...
GATE-INDEX-002: Header de auto-geração intacto
  cmd: python -c "import pathlib; idx=pathlib.Path('docs/hbtrack/ars/_INDEX.md').read_text(encoding='utf-8'); assert 'Auto-gerado' in idx,'FAIL'; print('PASS: header ok')"
  PASS: 'PASS: header ok'
  FAIL: 'AssertionError: FAIL'

GATE-INDEX-003: Contagem exata (sem linhas fantasma)
  cmd: python -c "import pathlib,re; ar_dir=pathlib.Path('docs/hbtrack/ars'); n_files=len([f for f in ar_dir.glob('AR_*.md') if '_INDEX' not in f.name]); idx=(ar_dir/'_INDEX.md').read_text(encoding='utf-8'); n_rows=len([l for l in idx.splitlines() if l.startswith('| AR_')]); assert n_rows==n_files,f'FAIL: {n_rows} rows vs {n_files} files'; print(f'PASS: {n_rows}=={n_files}')"
  PASS: 'PASS: N==N'
