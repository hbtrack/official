...
O campo `status` de `training_session` DEVE obedecer à state machine canônica definida em **ADR-017** (LAC-001 — RESOLVIDO por ADR-017 em 2026-03-15):
...
**Estados (7):** `DRAFT`, `SCHEDULED`, `PUBLISHED`, `IN_PROGRESS`, `COMPLETED`, `CANCELLED`, `ARCHIVED`.
**Estado inicial:** `DRAFT`.
**Estados terminais:** `COMPLETED` (→ `ARCHIVED`), `CANCELLED`.

> **Nota de migração (v0.x → v1.0):** A implementação atual (INV-TRAIN-006) usa os estados operacionais `draft/scheduled/in_progress/pending_review/readonly`. O mapeamento para os estados canônicos e o cronograma de migração estão definidos em ADR-017 §Migração.
...
|---|---|---|---|
| Invariantes do módulo | `docs/hbtrack/modulos/training/INVARIANTS_TRAINING.md` | Existente — **requer atualização (v1.0)** | INV-TRAIN-006 DEVE refletir state machine canônica ADR-017 (7 estados: DRAFT/SCHEDULED/PUBLISHED/IN_PROGRESS/COMPLETED/CANCELLED/ARCHIVED) |
...
**Resolução:** `ADR-017-training-session-state-machine.md` (2026-03-15). A state machine canônica de `training_session` é a definida em TRAIN-DEC-026 (`ARCH-DEC-TRAIN.md`): 7 estados — `DRAFT`, `SCHEDULED`, `PUBLISHED`, `IN_PROGRESS`, `COMPLETED`, `CANCELLED`, `ARCHIVED`. Os estados operacionais v0.x (`pending_review`, `readonly`) são descontinuados conforme plano de migração do ADR-017. Estado `PLANNED` do DOMAIN_AXIOMS.json global é removido da state machine de `training_session` no ciclo v1.0.

**Artefatos a atualizar (milestone v1.0):** `INVARIANTS_TRAINING.md` (INV-TRAIN-006); `DOMAIN_AXIOMS.json` global; `contracts/openapi/paths/training.yaml` (campo `status` → enum restrito).
