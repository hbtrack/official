...
- Em caso de conflito com qualquer outra convenção/documento, prevalece `.contract_driven/templates/API_RULES/API_RULES.yaml` (salvo exceção HB Track explícita e normativa).
...
4. `.contract_driven/templates/API_RULES/API_RULES.yaml` (**SSOT de API**)
...
- Convenções/validações/templates (SSOT): `.contract_driven/templates/API_RULES/API_RULES.yaml`
...
- `.contract_driven/templates/API_RULES/API_RULES.yaml` → `hbtrack_api_rules.legacy_rule_registry`
