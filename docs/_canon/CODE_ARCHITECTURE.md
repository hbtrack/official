...
> **v1.1.0 (2026-03-17):** Stack atualizada para Django Ninja (ADR-031 substitui §1 do ADR-026).
...
> **STATUS**: estrutura planejada — ainda não implementada no workspace.
> Quando implementar, usar o layout abaixo como guia autoritativo.
...
Quando ativo, verifica conformidade com esta arquitetura no pipeline CI:
1. `src/` existe
2. `docs/_canon/decisions/ADR-026-code-architecture.md` existe
3. Para módulos com status `implementation_ready` no MODULE_REGISTRY:
