...
**Nota importante (APIs)**: convenções, regras de validação e templates de **API HTTP/OpenAPI** têm SSOT em `.contract_driven/templates/API_RULES/API_RULES.yaml` (conforme `.contract_driven/CONTRACT_SYSTEM_RULES.md`). `docs/_canon/API_CONVENTIONS.md` existe como guia/ponteiro.
...
| 7 | `API_CONVENTIONS.md` | Guia/ponteiro de API — SSOT em `.contract_driven/templates/API_RULES/API_RULES.yaml` |
...
6. **`.contract_driven/templates/API_RULES/API_RULES.yaml`** — SSOT de convenções/validações/templates de API HTTP (OpenAPI/JSON/URLs/paginação/erros/segurança)
