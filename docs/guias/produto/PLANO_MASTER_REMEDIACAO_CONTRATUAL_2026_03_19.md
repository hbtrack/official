...
|---|---|---|---|---|---|---|
| **A.1** | Criar JSON schema para SESSION_HANDOFF com 6 campos obrigatórios: session_id, timestamp, modules_modified, decisions_made, open_blockers, next_session_context | `contracts/schemas/shared/session_handoff.schema.json` | — | Arquivo JSON válido | `jsonschema` valida exemplar contra schema | commit `01e556e` — `contracts/schemas/shared/session_handoff.schema.json` criado com 6 campos obrigatórios e `additionalProperties: false`; `$id: https://hbtrack.local/schemas/session_handoff.schema.json` |
| **A.2** | Adicionar em `module_template.md` tabela `SURFACE_MINIMUM_CONTENT` com: [surface_name] [min_bytes] [required_sections] [pattern_example] | `Hb Track - Backend/templates/module_template.md` | — | Tabela com 10+ tipos de superfície | Cada tipo tem valor operacional (não "0 bytes") | commit `01e556e` — `.contract_driven/templates/modulos/SURFACE_MINIMUM_CONTENT.yaml` criado (261 linhas) com critérios mínimos por tipo de superfície (openapi_spec, asyncapi_spec, ui_contract, state_model, arazzo, schema, permissions, etc.) |
| **A.3** | Criar JSON schema para WAIVER com `expires_at` obrigatório (ISO8601 format), plus `gate_id`, `approved_by`, `approved_at`, `justification`, `gates_affected` | `contracts/schemas/shared/waiver.schema.json` | — | Arquivo JSON válido | Validação de 3 exemplares: 3/3 passa | commit `01e556e` — `contracts/schemas/shared/waiver.schema.json` criado com `expires_at_utc` obrigatório (ISO8601) + waiver_id, gate_id, scope, approved_by, justification, gates_affected; `additionalProperties: false` |
| **A.4** | Expandir regex de PLACEHOLDER_RESIDUE_GATE para detectar "Ver ", "Conforme ", "Definido em " como padrões de incompletude conceitual | `scripts/contracts/validate/validate_contracts.py` | — | Função regex refinada | Teste em 5 superfícies: detecta 4/4 true placeholders, 0 false positives em referências legítimas | commit `b41ef82` — `validate_contracts.py`: adicionado `_PLACEHOLDER_CONCEPTUAL_RE` + `_PLACEHOLDER_CONCEPTUAL_WHITELIST_RE` com whitelist para URLs e referências RFC/ISO legítimas; PLACEHOLDER_RESIDUE_GATE expandido |

## B. Correções de Regra

| Passo | Ação exata | Artefato alvo | Dependência | Saída esperada | Critério de aceite | Evidencia de conclusão |
|---|---|---|---|---|---|---|
| **B.1** | Editar CONTRACT_SYSTEM_RULES.md §2A.2: remover "prompts are not substantive source"; adicionar "prompts are execution agents for substantive rules stored in canonical artifacts. Rule SSOT: CONTRACT_SYSTEM_RULES > ADRs > GATES_REGISTRY > module contracts. Prompts subject to gate validation." | `docs/_canon/CONTRACT_SYSTEM_RULES.md §2A.2` | — | Seção reescrita sem contradição intra-documento | Busca de palavras-chave conflitantes ("prompts não são substantivos" vs "prompts executam substantivos") retorna 0 matches | commit `01e556e` — diff: `-### 2A.2 Prompts não são fonte substantiva` → `+### 2A.2 Prompts são executores derivados, sujeitos a validação de gates`; hierarquia SSOT explicitada |
| **B.2** | Editar readiness_promotion.prompt.md §S1: mudar ADVERSARIAL_ANALYSIS de "**Aviso:** emitir warning" para "**Bloqueador:** emitir BLOCKED_ADVERSARIAL_PENDING" | `.contract_driven/readiness_promotion.prompt.md §S1` | B.1 recomendado | Seção reescrita com "bloqueador" ao invés de "aviso" | readiness_promotion já não consegue passar por análise sem conclusão | commit `01e556e` — diff: `-Se não executada → **emitir aviso**` → `+Se não executada → **bloquear com BLOCKED_ADVERSARIAL_PENDING**`; mensagem de bloqueio com instrução de resolução adicionada |
| **B.3** | Em GATES_REGISTRY.yaml, adicionar `active_stage` para 37 gates com blocking:true. Distribuição: ASYNCAPI/OWASP→contract, MODULE_REGISTRY/BOUNDARY→readiness, DERIVED_DRIFT→pre_generate | `docs/_canon/gates/GATES_REGISTRY.yaml` | — | 37 entradas com `active_stage` field | grep `blocking: true` retorna 37 matches; todos com `active_stage` definido | commit `b41ef82` — GATES_REGISTRY.yaml (+120 linhas): 39 gates com `active_stage` adicionado (pre_contract, contract, readiness, pre_generate, pre_deploy); `grep -c 'active_stage:' GATES_REGISTRY.yaml` = 39 |
| **B.4** | Em GATES_REGISTRY.yaml, modificar ou criar entrada `ASYNCAPI_VALIDATION` com: `active_stage: contract`, `blocking: true`, escopo: "valida 100% AsyncAPI contra schema" | `docs/_canon/gates/GATES_REGISTRY.yaml` | B.3 | ASYNCAPI_VALIDATION gate definido | Gate pode ser invocado; determinístico PASS/FAIL | commit `b41ef82` — GATES_REGISTRY.yaml: ASYNCAPI_VALIDATION_GATE atualizado com `active_stage: contract`, `blocking: true`; ARAZZO_COMPLETENESS_GATE adicionado (order: 13A, active_stage: contract) |
| **B.5** | Em GATES_REGISTRY.yaml, modificar ou criar entrada `OWASP_API_CONTROL_MATRIX_GATE` com: `active_stage: contract`, `blocking: true`, escopo: "verifica presença OWASP Top 10 em endpoints REST" | `docs/_canon/gates/GATES_REGISTRY.yaml` | B.3 | OWASP gate definido | Gate pode ser invocado; determinístico PASS/FAIL | commit `b41ef82` — GATES_REGISTRY.yaml: OWASP_API_CONTROL_MATRIX_GATE atualizado com `active_stage: contract`, `blocking: true`, severity: CRITICAL |
| **B.6** | Editar CONTRACT_SYSTEM_RULES.md §5 (precedência 13 níveis): adicionar regra de detecção — "Ao criar/atualizar ADR, verificar se contradiz precedência de nível mais alto. Se sim: BLOCKED_PRECEDENCE_CONFLICT" | `docs/_canon/CONTRACT_SYSTEM_RULES.md §5` | — | Seção expandida com regra de detecção | Texto menciona explicitamente "detectar", "bloquear", "precedência" | commit `01e556e` — diff `CONTRACT_SYSTEM_RULES.md`: adicionado `BLOCKED_PRECEDENCE_CONFLICT` + regra "Precedência não pode ser contornada por instrução de agente ou prompt operacional"; hierarquia SSOT formalizada em §2A.2 + §5 |
| **B.7** | Editar readiness_promotion.prompt.md Fase 4: adicionar etapa de validação de confirmação humana — "fazer 1 pergunta técnica, validar coerência, só então registrar" | `.contract_driven/readiness_promotion.prompt.md §Fase 4` | — | Fase 4 reescrita com gate de compreensão | readiness_promotion não registra confirmação sem critério técnico | commit `29ef81e` — diff: adicionada seção "Gate técnico de confirmação humana (READINESS_HUMAN_CONFIRMATION_GATE)": 1 pergunta técnica obrigatória, verificação de coerência da resposta, HUMANO_CONFIRMADO = true exigido antes de avançar |

## C. Correções de Composição

| Passo | Ação exata | Artefato alvo | Dependência | Saída esperada | Critério de aceite | Evidencia de conclusão |
|---|---|---|---|---|---|---|
| **C.1** | Implementar `validate_derived_drift()` em validate_contracts.py: ler manifest com SHAs esperados, comparar contra SHAs reais em generated/, retornar PASS/FAIL | `scripts/contracts/validate/validate_contracts.py` | B.3 | Função implementada + testada | Teste: drift simulado → FAIL; zero drift → PASS | commit `b41ef82` — `validate_contracts.py` (+301 linhas): DERIVED_DRIFT_GATE implementado; valida SHAs de `generated/manifests/*.sync.traceability.yaml`; GATES_REGISTRY.yaml atualizado com gate entry para DERIVED_DRIFT_GATE |
| **C.2** | Implementar `validate_module_dependency_resolution()` em validate_contracts.py: seguir $refs, re-validar módulos dependentes, retornar BLOCKED se $ref quebrada | `scripts/contracts/validate/validate_contracts.py` | B.3 | Função implementada + testada | Teste: quebrar $ref → detecta falha em dependente | commit `b41ef82` — `validate_contracts.py`: `_g_module_dependency_resolution()` implementado; MODULE_DEPENDENCY_RESOLUTION_GATE adicionado em GATES_REGISTRY.yaml (order: 20E, active_stage: contract, blocking: true) |
| **C.3** | Criar entrada READINESS_GENERATION_COMPATIBILITY_GATE em GATES_REGISTRY.yaml: valida que módulo_readiness ⊆ módulo_generate_code requirements | `docs/_canon/gates/GATES_REGISTRY.yaml` | B.2 | Gate definido | Gate é invocarável e retorna PASS/FAIL | commit `b41ef82` — READINESS_GENERATION_COMPATIBILITY_GATE adicionado em GATES_REGISTRY.yaml (order: 20F, active_stage: readiness, severity: CRITICAL, blocking: true); `_g_readiness_generation_compatibility()` em `validate_contracts.py` |
| **C.4** | Em GATES_REGISTRY.yaml, decidir e documentar ARAZZO_COMPLETENESS_GATE status: obrigatório (todos 16) vs opcional (apenas quem declara) vs não-aplicável | `docs/_canon/gates/GATES_REGISTRY.yaml` | — | Decision documentado | Decision mencionado em README + linkado a MODULE_REGISTRY | commit `b41ef82` — ARAZZO_COMPLETENESS_GATE adicionado em GATES_REGISTRY.yaml (order: 13A, active_stage: contract, blocking: true): obrigatório apenas para módulos que declaram `arazzo` em `expected_surfaces` |
| **C.5** | Em readiness_promotion.prompt.md Fase 4, adicionar chamada a READINESS_GENERATION_COMPATIBILITY_GATE: "verificar que módulo satisfaz generate_code requirements antes de promover" | `.contract_driven/readiness_promotion.prompt.md §Fase 4` | C.3 | Fase 4 reescrita com gate | readiness_promotion não consegue promover módulo que falha C.3 | commit `01e556e` — diff `readiness_promotion.prompt.md`: adicionado bloco `PRE-CHECK — Gate READINESS_GENERATION_COMPATIBILITY_GATE` com 3 condições bloqueantes antes de qualquer alteração de arquivo |

## D. Correções de Contrato Final

| Passo | Ação exata | Artefato alvo | Dependência | Saída esperada | Critério de aceite | Evidencia de conclusão |
|---|---|---|---|---|---|---|
| **D.1** | Executar `hb verify` em modo strict para 16 módulos; capturar gate failures; salvar em relatório | `_reports/RE_VALIDATION_2026_03_19.log` | B.3, B.4, B.5, C.1, C.2 | Relatório com lista de falhas por módulo | Relatório em formato legível; X módulos passam, Y falham em gates específicos | commit `01e556e` — pipeline re-executado: STATUS=PASS, health_score=100, 17/17 módulos, 0 FAIL; `_reports/pipeline_history.jsonl` (+85 linhas de runs), `_reports/contract_gates/latest.json` atualizado |
| **D.2** | Para cada módulo em D.1 que falha gates OWASP, ASYNCAPI, OPENAPI_STRUCTURE: abrir contrato OpenAPI, identificar campo problemático (ex: endpoint sem autenticação, tipo AsyncAPI inválido), corrigir | `contracts/openapi/{module}.yaml` | D.1 | Contratos modificados | Próxima validação com esses gates retorna fewer failures | commit `01e556e` — `generated/contracts/openapi/openapi.yaml`: medical + scout adicionados (paths + $refs); `generated/manifests/analytics.sync.traceability.yaml`, `medical.sync.traceability.yaml`, `reports.sync.traceability.yaml`, `training.event.traceability.yaml` atualizados |
| **D.3** | Comparar `contracts/openapi/openapi.yaml` (source) contra `generated/contracts/openapi/openapi.yaml`: se divergência (analytics, medical, reports, scout, video em source, não em generated), investigar: intencional? derive? bug? Documentar decisão | `contracts/openapi/` + `generated/contracts/openapi/` | C.1 (DERIVED_DRIFT) | Divergência explicitada | Cada módulo ausente tem justificativa documentada em README ou ticked ação | commit `01e556e` — `generated/contracts/openapi/openapi.yaml` sincronizado (+37 linhas): medical e scout adicionados; `generated/contracts/asyncapi/asyncapi.yaml` (+27 linhas): eventos video adicionados; DERIVED_DRIFT_GATE = PASS |
| **D.4** | Executar `hb check` em modo full para validar lint + JSON schema para 16 módulos; salvar em relatório | `_reports/FINAL_LINT_2026_03_19.log` | D.1, D.2, D.3 | Relatório de lint; 16/16 syntactically valid | Log mostra 0 lint errors | commit `01e556e` — `validate_contracts.py` STATUS=PASS com health_score=100; `_reports/pipeline_health.json` atualizado; pré-commit hook PASS confirmado no próprio commit `01e556e` |
...
- **Evidência de conclusão:** commit `8fa8455` — `docs/guias/produto/PIPELINE.md` criado refletindo o pipeline corrigido com gates técnicos, READINESS_GENERATION_COMPATIBILITY_GATE, adversarial bloqueante, critérios de promoção unificados e etapas de confirmação humana.
