...
```
python scripts/checks/check_plans_ar_sync.py && python -c "import json; result=json.load(open('_reports/audit/PLANS_AR_SYNC/result.json')); assert result['status']=='PASS', f'FAIL: {result}'"
```
...
1. **Criar scripts/checks/check_plans_ar_sync.py**: Script Python que valida a sincronização entre planos JSON e ARs materializadas.
   ...
   - Gera report JSON em `_reports/audit/PLANS_AR_SYNC/result.json`.
...
**Comando**: `python scripts/checks/check_plans_ar_sync.py && python -c "import json; result=json.load(open('_reports/audit/PLANS_AR_SYNC/result.json')); assert result['status']=='PASS', f'FAIL: {result}'"`
**Exit Code**: 0
