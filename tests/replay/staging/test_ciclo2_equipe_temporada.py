"""
Ciclo 2 — Equipe e Temporada: testes estruturais + live staging (opcional).
"""
from __future__ import annotations

import pytest
from replay import replay_team_season as mod


class TestStructure:
    def test_cycle_id(self):
        assert mod.CYCLE_ID == "ciclo2_equipe_temporada"

    def test_modules_declared(self):
        assert "teams" in mod.CYCLE_MODULES
        assert "seasons" in mod.CYCLE_MODULES
    ...
    def test_has_team_creation_endpoint(self):
        names = [ep["name"] for ep in mod.ENDPOINTS]
        assert "teams_create" in names

    def test_has_season_creation_endpoint(self):
        names = [ep["name"] for ep in mod.ENDPOINTS]
        assert "seasons_create" in names
