...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "wellness" / "graph"
...
def test_wellness_graph_files_exist_and_are_active():
    expected = {
...
def test_wellness_module_manifest_refs_resolve():
...
def test_wellness_entities_graph_matches_schema_and_runtime():
    entities = _load_yaml(GRAPH_ROOT / "entity_graph.yaml")["entities"]
    entity = entities["WellnessEntry"]
    ...
    assert entity["schema_ref"].endswith("wellness_entry.schema.json")
...
def test_wellness_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "wellness.yaml")
...
def test_wellness_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_wellness_test_obligations_cover_graph_contracts_and_runtime():
...
def test_wellness_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "wellness" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "wellness" / "DOMAIN_RULES_WELLNESS.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "wellness" / "TEST_MATRIX_WELLNESS.md").read_text(encoding="utf-8")
    ...
    wellness_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_WELLNESS_GRAPH")
    covered = set()
    for pattern in wellness_graph_entry["path_globs"]:
        covered |= {
    ...
    assert covered == {
        ...
        "docs/hbtrack/modulos/wellness/graph/test_obligations.yaml",
        "docs/hbtrack/modulos/wellness/graph/openapi_paths.yaml",
    }
