...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "seasons" / "graph"
...
def test_seasons_graph_files_exist_and_are_active():
    expected = {
...
def test_seasons_module_manifest_refs_resolve():
...
def test_seasons_entities_graph_matches_schema_and_runtime():
    ...
    assert entity["schema_ref"].endswith("season.schema.json")
...
def test_seasons_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "seasons.yaml")
...
def test_seasons_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_seasons_test_obligations_cover_graph_contracts_and_runtime():
...
def test_seasons_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "seasons" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "seasons" / "DOMAIN_RULES_SEASONS.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "seasons" / "TEST_MATRIX_SEASONS.md").read_text(encoding="utf-8")
    ...
    seasons_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_SEASONS_GRAPH")
    covered = set()
    for pattern in seasons_graph_entry["path_globs"]:
        covered |= {
    ...
    assert covered == {
        "docs/hbtrack/modulos/seasons/graph/module_manifest.yaml",
        "docs/hbtrack/modulos/seasons/graph/entity_graph.yaml",
