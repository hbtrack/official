...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "reports" / "graph"
...
def test_reports_graph_files_exist_and_are_active():
    expected = {
...
def test_reports_module_manifest_refs_resolve():
    ...
    assert manifest["phase"] == "source_graph_pilot"
    assert manifest["known_gaps"]


def test_reports_entities_graph_matches_schema_and_runtime():
    entities = _load_yaml(GRAPH_ROOT / "entity_graph.yaml")
    report_job = entities["entities"]["ReportJob"]
    schema = _load_json(_resolve(report_job["schema_ref"]))
    entity_file, symbol = _resolve_symbol(report_job["runtime_entity_ref"])
    entity_source = entity_file.read_text(encoding="utf-8")

    assert f"class {symbol}" in entity_source

    graph_sovereign_names = [field["name"] for field in report_job["sovereign_fields"]]
    assert graph_sovereign_names == list(schema["properties"].keys())

    graph_required = sorted(field["name"] for field in report_job["sovereign_fields"] if field["required"])
    assert graph_required == sorted(schema["required"])

    for field in report_job["sovereign_fields"]:
        assert field["runtime_name"] in entity_source

    expected_runtime_extensions = {"statusLabel", "completedAt", "errorMessage"}
    graph_runtime_extensions = {field["name"] for field in report_job["runtime_extension_fields"]}
    assert graph_runtime_extensions == expected_runtime_extensions
    for field in report_job["runtime_extension_fields"]:
        assert field["runtime_name"] in entity_source


def test_reports_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "reports.yaml")
    api_source = (REPO_ROOT / "src" / "reports" / "api.py").read_text(encoding="utf-8")
    use_case_source = (REPO_ROOT / "src" / "reports" / "application" / "use_cases.py").read_text(encoding="utf-8")
    ...
    for entry in endpoints:
        assert entry["method"] in {"GET", "POST", "PATCH"}
        ...
        assert handler_file == REPO_ROOT / "src" / "reports" / "api.py"
        assert f"def {handler_symbol}" in api_source
        use_case_file, use_case_symbol = _resolve_symbol(entry["use_case_ref"])
        assert use_case_file == REPO_ROOT / "src" / "reports" / "application" / "use_cases.py"
        assert f"class {use_case_symbol}" in use_case_source


def test_reports_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    operation_ids = {entry["operation_id"] for entry in endpoints}
    errors = _load_yaml(GRAPH_ROOT / "errors.yaml")["errors"]
    rules_source = (REPO_ROOT / "src" / "reports" / "domain" / "rules.py").read_text(encoding="utf-8")
    api_source = (REPO_ROOT / "src" / "reports" / "api.py").read_text(encoding="utf-8")
    entity_source = (REPO_ROOT / "src" / "reports" / "domain" / "entities.py").read_text(encoding="utf-8")
    ...
    for error in errors:
        ...
        if "exception_ref" in error:
            ...
            assert exc_file == REPO_ROOT / "src" / "reports" / "domain" / "rules.py"
            assert f"class {exc_symbol}" in rules_source
        if "source_ref" in error:
            src_file, src_symbol = _resolve_symbol(error["source_ref"])
            source = api_source if src_file.name == "api.py" else entity_source
            assert src_file.exists()
            assert src_symbol in source


def test_reports_test_obligations_cover_graph_contracts_and_runtime():
...
def test_reports_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "reports" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "reports" / "DOMAIN_RULES_REPORTS.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "reports" / "TEST_MATRIX_REPORTS.md").read_text(encoding="utf-8")
    ...
    reports_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_REPORTS_GRAPH")
    covered = set()
    for pattern in reports_graph_entry["path_globs"]:
        covered |= {
    ...
    assert covered == {
        ...
        "docs/hbtrack/modulos/reports/graph/openapi_paths.yaml",
    }
