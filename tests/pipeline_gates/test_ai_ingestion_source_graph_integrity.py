...
def test_ai_ingestion_module_docs_and_manifest_reference_source_graph():
    ...
    ai_ingestion_graph_entry = next(
        (entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_AI_INGESTION_GRAPH"),
        None,
    )
    assert ai_ingestion_graph_entry is not None, "HBTRACK_AI_INGESTION_GRAPH ausente do DOC_USAGE_MANIFEST"
