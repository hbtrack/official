...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "ai_ingestion" / "graph"
...
def test_ai_ingestion_graph_files_exist_and_are_active():
    expected = {
...
def test_ai_ingestion_module_manifest_refs_resolve():
...
def test_ai_ingestion_entities_graph_matches_schema_and_runtime():
    ...
    assert entity["schema_ref"].endswith("ingestion_job.schema.json")
    ...
    for field in entity["runtime_extension_fields"]:
        assert field["runtime_name"] in entity_source


def test_ai_ingestion_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "ai_ingestion.yaml")
...
def test_ai_ingestion_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_ai_ingestion_test_obligations_cover_graph_contracts_and_runtime():
...
def test_ai_ingestion_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "ai_ingestion" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "ai_ingestion" / "DOMAIN_RULES_AI_INGESTION.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "ai_ingestion" / "TEST_MATRIX_AI_INGESTION.md").read_text(encoding="utf-8")
    ...
    ai_ingestion_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_AI_INGESTION_GRAPH")
    covered = set()
    for pattern in ai_ingestion_graph_entry["path_globs"]:
        covered |= {
    ...
    assert covered == {
        ...
        "docs/hbtrack/modulos/ai_ingestion/graph/entities.yaml",
        "docs/hbtrack/modulos/ai_ingestion/graph/endpoints.yaml",
        "docs/hbtrack/modulos/ai_ingestion/graph/errors.yaml",
        "docs/hbtrack/modulos/ai_ingestion/graph/test_obligations.yaml",
    }
