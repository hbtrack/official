...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "identity_access" / "graph"
...
def test_identity_access_graph_files_exist_and_are_active():
    expected = {
    ...
    for filename in expected:
        ...
        assert payload["module"] == "identity_access"


def test_identity_access_module_manifest_refs_resolve():
...
def test_identity_access_entities_graph_matches_schema_and_runtime():
    entities = _load_yaml(GRAPH_ROOT / "entity_graph.yaml")["entities"]

    entity = entities["AuthSession"]
...
def test_identity_access_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "identity_access.yaml")
...
def test_identity_access_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_identity_access_test_obligations_cover_graph_contracts_and_runtime():
...
def test_identity_access_module_docs_and_manifest_reference_source_graph():
    manifest = _load_yaml(REPO_ROOT / "docs" / "_canon" / "DOC_USAGE_MANIFEST.yaml")

    identity_access_graph_entry = next(
        (entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_IDENTITY_ACCESS_GRAPH"),
        None,
    )
    assert identity_access_graph_entry is not None, "HBTRACK_IDENTITY_ACCESS_GRAPH ausente do DOC_USAGE_MANIFEST"
