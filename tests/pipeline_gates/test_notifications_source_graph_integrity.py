...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "notifications" / "graph"
...
def test_notifications_graph_files_exist_and_are_active():
    expected = {
...
def test_notifications_module_manifest_refs_resolve():
...
def test_notifications_entities_graph_matches_schema_and_runtime():
    entities = _load_yaml(GRAPH_ROOT / "entities.yaml")["entities"]
    entity = entities["NotificationDelivery"]
    ...
    assert entity["schema_ref"].endswith("notification_delivery.schema.json")
...
def test_notifications_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "notifications.yaml")
...
def test_notifications_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_notifications_test_obligations_cover_graph_contracts_and_runtime():
...
def test_notifications_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "notifications" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "notifications" / "DOMAIN_RULES_NOTIFICATIONS.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "notifications" / "TEST_MATRIX_NOTIFICATIONS.md").read_text(encoding="utf-8")
    ...
    notifications_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_NOTIFICATIONS_GRAPH")
    covered = set()
    for pattern in notifications_graph_entry["path_globs"]:
        covered |= {
