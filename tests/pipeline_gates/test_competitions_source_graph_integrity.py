...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "competitions" / "graph"
...
def test_competitions_graph_files_exist_and_are_active():
    expected = {
...
def test_competitions_module_manifest_refs_resolve():
...
def test_competitions_entities_graph_matches_schema_and_runtime():
    ...
    assert entity["schema_ref"].endswith("competition.schema.json")
...
def test_competitions_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "competitions.yaml")
...
def test_competitions_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_competitions_test_obligations_cover_graph_contracts_and_runtime():
...
def test_competitions_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "competitions" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "competitions" / "DOMAIN_RULES_COMPETITIONS.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "competitions" / "TEST_MATRIX_COMPETITIONS.md").read_text(encoding="utf-8")
    ...
    competitions_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_COMPETITIONS_GRAPH")
    covered = set()
    for pattern in competitions_graph_entry["path_globs"]:
        covered |= {
    ...
    assert covered == {
        ...
        "docs/hbtrack/modulos/competitions/graph/entity_graph.yaml",
