...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "analytics" / "graph"
...
def test_analytics_graph_files_exist_and_are_active():
    expected = {
...
def test_analytics_module_manifest_refs_resolve():
...
def test_analytics_entities_graph_matches_schema_and_runtime():
    entities = _load_yaml(GRAPH_ROOT / "entity_graph.yaml")["entities"]

    expected_entities = {
        "AnalyticsSnapshot": "analytics_snapshot.schema.json",
        "AnalyticsQueryRequest": "analytics_query_request.schema.json",
    }

    for entity_name, schema_name in expected_entities.items():
        ...
        assert entity["schema_ref"].endswith(schema_name)
    ...
    assert {field["name"] for field in entities["AnalyticsSnapshot"]["runtime_extension_fields"]} == {"createdByUserId"}


def test_analytics_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "analytics.yaml")
...
def test_analytics_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_analytics_test_obligations_cover_graph_contracts_and_runtime():
...
def test_analytics_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "analytics" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "analytics" / "DOMAIN_RULES_ANALYTICS.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "analytics" / "TEST_MATRIX_ANALYTICS.md").read_text(encoding="utf-8")
    ...
    analytics_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_ANALYTICS_GRAPH")
    covered = set()
    for pattern in analytics_graph_entry["path_globs"]:
        covered |= {
    ...
    assert covered == {
        ...
        "docs/hbtrack/modulos/analytics/graph/test_obligations.yaml",
        "docs/hbtrack/modulos/analytics/graph/openapi_paths.yaml",
    }
