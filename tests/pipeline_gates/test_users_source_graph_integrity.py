...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "users" / "graph"
...
def test_users_graph_files_exist_and_are_active():
    expected = {
...
def test_users_module_manifest_refs_resolve():
...
def test_users_entities_graph_matches_schema_and_runtime():
    entities = _load_yaml(GRAPH_ROOT / "entity_graph.yaml")["entities"]
    entity = entities["UserProfile"]
    ...
    assert entity["schema_ref"].endswith("user_profile.schema.json")
...
def test_users_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "users.yaml")
...
def test_users_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_users_test_obligations_cover_graph_contracts_and_runtime():
...
def test_users_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "users" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "users" / "DOMAIN_RULES_USERS.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "users" / "TEST_MATRIX_USERS.md").read_text(encoding="utf-8")
    ...
    users_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_USERS_GRAPH")
    covered = set()
    for pattern in users_graph_entry["path_globs"]:
        covered |= {
