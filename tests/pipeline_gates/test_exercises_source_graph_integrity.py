...
GRAPH_ROOT = REPO_ROOT / "docs" / "hbtrack" / "modulos" / "exercises" / "graph"
...
def test_exercises_graph_files_exist_and_are_active():
    expected = {
...
def test_exercises_module_manifest_refs_resolve():
...
def test_exercises_entities_graph_matches_schema_and_runtime():
    entities = _load_yaml(GRAPH_ROOT / "entity_graph.yaml")["entities"]

    expected_entities = {
        "Exercise": "exercise.schema.json",
        "ExerciseVersion": "exercise_version.schema.json",
        "ExerciseRelation": "exercise_relation.schema.json",
        "ExerciseAcl": "exercise_acl.schema.json",
    }
...
def test_exercises_endpoints_graph_matches_openapi_and_runtime_refs():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
    openapi_paths = _load_yaml(REPO_ROOT / "contracts" / "openapi" / "paths" / "exercises.yaml")
...
def test_exercises_errors_graph_maps_to_real_exceptions_and_operations():
    endpoints = _load_yaml(GRAPH_ROOT / "endpoints.yaml")["endpoints"]
...
def test_exercises_test_obligations_cover_graph_contracts_and_runtime():
...
def test_exercises_module_docs_and_manifest_reference_source_graph():
    readme = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "exercises" / "README.md").read_text(encoding="utf-8")
    domain_rules = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "exercises" / "DOMAIN_RULES_EXERCISES.md").read_text(encoding="utf-8")
    test_matrix = (REPO_ROOT / "docs" / "hbtrack" / "modulos" / "exercises" / "TEST_MATRIX_EXERCISES.md").read_text(encoding="utf-8")
    ...
    exercises_graph_entry = next(entry for entry in manifest["entries"] if entry["rule_id"] == "HBTRACK_EXERCISES_GRAPH")
    covered = set()
    for pattern in exercises_graph_entry["path_globs"]:
        covered |= {
    ...
    assert covered == {
        ...
        "docs/hbtrack/modulos/exercises/graph/test_obligations.yaml",
        "docs/hbtrack/modulos/exercises/graph/openapi_paths.yaml",
    }
