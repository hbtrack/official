...
class TestInvTrain005Immutability60Days:
    ...
    def _create_mock_session(self, days_ago: int, status: str = "readonly") -> MagicMock:
        """Cria mock de sessão com session_at X dias atrás."""
        mock_session = MagicMock()
        mock_session.status = status
        mock_session.session_at = datetime.now(timezone.utc) - timedelta(days=days_ago)
