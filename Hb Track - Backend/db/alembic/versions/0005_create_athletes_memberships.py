...
def upgrade():
    ...
    # ========== MEDICAL_CASES ========== 
    # R12/R13: casos médicos de atletas
    op.create_table(
        'medical_cases',
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        sa.Column('athlete_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('reason', sa.String(500), nullable=True),
        sa.Column('status', sa.String(50), server_default=sa.text("'ativo'"), nullable=True),
        ...
        sa.ForeignKeyConstraint(['athlete_id'], ['athletes.id'], name='fk_medical_cases_athlete_id', ondelete='RESTRICT'),
        sa.ForeignKeyConstraint(['organization_id'], ['organizations.id'], name='fk_medical_cases_organization', ondelete='RESTRICT'),
        sa.ForeignKeyConstraint(['created_by_user_id'], ['users.id'], name='fk_medical_cases_created_by_user', ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('id', name='pk_medical_cases'),
        sa.CheckConstraint(
            "(deleted_at IS NULL AND deleted_reason IS NULL) OR (deleted_at IS NOT NULL AND deleted_reason IS NOT NULL)",
            name='ck_medical_cases_deleted_reason'
        ),
        sa.CheckConstraint(
            "status IN ('ativo', 'resolvido', 'em_acompanhamento')",
            name='medical_cases_status_check'
        ),
        comment='Casos médicos de atletas. V1.2: RDB4 compliant (soft delete + deleted_reason).'
    )

    op.create_index('idx_medical_cases_athlete', 'medical_cases', ['athlete_id'])
    op.create_index('idx_medical_cases_organization_id', 'medical_cases', ['organization_id'])
