...
def validate_profile(profile_path: str) -> list[str]:
    ...
    # L2: validate required SSOT artifacts exist
    ssot_errors = validate_ssot_artifacts(data, p)
    errors.extend(ssot_errors)

    return errors


def validate_ssot_artifacts(data: dict, profile_path: Path) -> list[str]:
    """Validate that required SSOT artifacts exist on filesystem."""
    errors = []
    ssot_registry = data.get("ssot_registry", [])
    
    if not isinstance(ssot_registry, list):
        return errors
    
    # Determine repo root (profile is at docs/_canon/HB_TRACK_PROFILE.yaml)
    repo_root = profile_path.parent.parent.parent
    
    for i, entry in enumerate(ssot_registry):
        if not isinstance(entry, dict):
            continue
        
        required = entry.get("required", False)
        artifact_path = entry.get("path", "")
        artifact_name = entry.get("artifact", f"index[{i}]")
        
        # Only check if required is true
        if required and artifact_path:
            full_path = repo_root / artifact_path
            if not full_path.exists():
                errors.append(f"L2: required artifact missing: {artifact_path} (artifact: {artifact_name})")
