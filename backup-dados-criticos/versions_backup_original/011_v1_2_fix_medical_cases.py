...
revision = '011_v1_2_fix_medical_cases'
down_revision = '010_v1_2_persons_normalized'
branch_labels = None
depends_on = None


def upgrade():
    # ========== 1. Add deleted_reason column (RDB4) ==========
    op.add_column('medical_cases',
        sa.Column('deleted_reason', sa.Text(), nullable=True)
    )
    
    # Add CHECK constraint for deleted_reason (RDB4)
    # deleted_reason is required when deleted_at is NOT NULL
    op.execute("""
        ALTER TABLE medical_cases
        ADD CONSTRAINT ck_medical_cases_deleted_reason
        CHECK (
    ...
    # ========== 2. Add updated_at trigger (RDB18.1) ==========
    op.execute("""
