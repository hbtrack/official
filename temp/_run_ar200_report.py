"""
Invoca hb report 200 com o validation_command exato da AR_200.
Evita problemas de escaping no PowerShell.
"""
import subprocess
import sys
import os
...
validation_command = (
    "python -c \"import os,sys; base='_reports/training'; ids=['INV-001','INV-002','INV-003','INV-004','INV-005','INV-008','INV-009','INV-030','INV-032','CONTRACT-077-085']; "
    ...
    "[sys.exit('FAIL: NOT_RUN persiste na linha '+lbl) for _,lbl in inv if t.find(lbl)!=-1 and 'NOT_RUN' in t[t.find('| '+lbl):t.find('| '+lbl)+450]]; "
    ...
    "[sys.exit('FAIL: NOT_RUN persiste na linha '+c) for c in ct_ids if t.find(c)!=-1 and 'NOT_RUN' in t[t.find('| '+c):t.find('| '+c)+450]]; "
