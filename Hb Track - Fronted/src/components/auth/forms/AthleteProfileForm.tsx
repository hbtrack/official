...
export interface AthleteFormData {
  full_name: string
  phone: string
  birth_date: string  // OBRIGATÓRIO para atletas no banco
  gender: string
}
...
export default function AthleteProfileForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validação: birth_date é obrigatório para atletas
    if (!birthDate) {
      return // Validação HTML5 já vai impedir
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <p className="text-slate-600 dark:text-slate-400 text-sm mb-6">
          Complete seu perfil como <span className="font-semibold">Atleta</span>.
        </p>
      </div>
      ...
      {/* Nota: Campos específicos como altura, peso e posições serão preenchidos depois na ficha do atleta */}
      <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
        <p className="text-sm text-slate-600 dark:text-slate-400">
          ℹ️ Dados adicionais do atleta (altura, peso, posições) serão preenchidos posteriormente na ficha completa.
        </p>
      </div>
